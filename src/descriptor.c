...
static int verify_multi(ms_node *node)
{
    ...
    if (count < 2 || count - 1 > DESCRIPTOR_MINISCRIPT_MULTI_MAX)
        return WALLY_EINVAL;

    top = node->child;
    if (!top->next || top->builtin || top->kind != KIND_NUMBER ||
        top->number <= 0 || count < (uint32_t)top->number)
        return WALLY_EINVAL;
...
{
    node->type_properties = verify_andor_property(
...
{
    node->type_properties = verify_or_i_property(
        node->child->type_properties,
        node->child->next->type_properties);
    return node->type_properties ? WALLY_OK : WALLY_EINVAL;
}
...
#define PROP_CHANGE(keep, set) *properties &= (TYPE_MASK | keep); *properties |= set
    ...
    if ((*properties & PROP_Z) && (*properties & PROP_O))
        return WALLY_EINVAL;
    if ((*properties & PROP_N) && (*properties & PROP_Z))
        return WALLY_EINVAL;
    if ((*properties & TYPE_V) && (*properties & PROP_D))
        return WALLY_EINVAL;
    if ((*properties & TYPE_K) && !(*properties & PROP_U))
        return WALLY_EINVAL;
    if ((*properties & TYPE_V) && (*properties & PROP_U))
        return WALLY_EINVAL;
    if ((*properties & PROP_E) && (*properties & PROP_F))
        return WALLY_EINVAL;
    if ((*properties & PROP_E) && !(*properties & PROP_D))
        return WALLY_EINVAL;
    if ((*properties & TYPE_V) && (*properties & PROP_E))
        return WALLY_EINVAL;
    if ((*properties & PROP_D) && (*properties & PROP_F))
        return WALLY_EINVAL;
    if ((*properties & TYPE_V) && !(*properties & PROP_F))
        return WALLY_EINVAL;
    if ((*properties & TYPE_K) && !(*properties & PROP_S))
        return WALLY_EINVAL;
    if ((*properties & PROP_Z) && !(*properties & PROP_M))
        return WALLY_EINVAL;
                         ...
                         unsigned char *script, size_t script_len, size_t *written)
{
    ...
    if (*written + 1 > DESCRIPTOR_REDEEM_SCRIPT_MAX_SIZE)
        return WALLY_EINVAL;
                           ...
                           unsigned char *script, size_t script_len, size_t *written)
{
    ...
    if (output_len > DESCRIPTOR_REDEEM_SCRIPT_MAX_SIZE)
        ret = WALLY_EINVAL;
...
{
    if (!*written || (*written + 1 > script_len) || (*written + 1 > DESCRIPTOR_WITNESS_SCRIPT_MAX_SIZE))
        return WALLY_EINVAL;
                          ...
                          unsigned char *script, size_t script_len, size_t *written)
{
    ...
    ret = generate_script(child, child_num, script, script_len, &offset);
    if (ret != WALLY_OK)
        return ret;
                        ...
                        unsigned char *script, size_t script_len, size_t *written)
{
    ...
    return *written > DESCRIPTOR_REDEEM_SCRIPT_MAX_SIZE ?  WALLY_EINVAL : ret;
}
                          ...
                          unsigned char *script, size_t script_len, size_t *written)
{
    int ret;
    size_t output_len = *written;
    if (!node->child || script_len < DESCRIPTOR_MIN_SIZE ||
        !node_is_root(node) || !node->builtin)
        return WALLY_EINVAL;
    ...
    if (output_len + 1 > DESCRIPTOR_REDEEM_SCRIPT_MAX_SIZE)
        return WALLY_EINVAL;
...
#define WRAP_REQUIRE(req, move_by) output_len = (req); \
    if (*written + output_len > script_len || *written + output_len > DESCRIPTOR_WITNESS_SCRIPT_MAX_SIZE) \
        return WALLY_EINVAL; \
...
/* TODO: Expose this */
static int bip32_path_from_string_n(const char *path, size_t path_len,
                                    bool is_private,
                                    ...
                                    size_t *written, size_t *wildcard_pos_out)
{
    ...
    *wildcard_pos_out = 0;
    ...
    for (i = 0; i < path_len; /*++i*/) {
        start = i;
        v = 0;
        while (path[i] >= '0' && path[i] <= '9' && i < path_len) {
            v = v * 10 + (path[i] - '0');
        ...
        if (i == start) {
            /* No number found */
            if (path[i] == '/') {
                if (i && (path[i - 1] < '0' || path[i - 1] > '9') &&
                    !is_hardened_indicator(path[i - 1]))
            ...
            if (path[i] == '*') {
                if ((i && path[i - 1] != '/') || *wildcard_pos_out)
                    goto fail; /* Duplicate wildcard or not after a slash */
                ++i;
                if (i < path_len && is_hardened_indicator(path[i])) {
                    if (!is_private)
                        goto fail; /* Hardened derivation in non-private mode */
                    ++i;
                    if (child_path)
                        child_path[*written] = BIP32_INITIAL_HARDENED_CHILD;
                } else {
                    if (child_path)
                        child_path[*written] = 0;
                }
        ...
        if (is_hardened_indicator(path[i])) {
            if (!is_private)
                goto fail; /* Hardened derivation in non-private mode */
        ...
        if (ret == WALLY_OK) {
            if (output_len == EC_PRIVATE_KEY_LEN + 2 && privkey[EC_PRIVATE_KEY_LEN + 1] == 1) {
                if (node->is_xonly_key) {
                    ...
                    *written = EC_PUBLIC_KEY_XONLY_LEN;
                } else {
                           ...
                           unsigned char *script, size_t script_len, size_t *written)
{
    ...
    ret = wally_addr_n_segwit_to_bytes(str, str_len, str, hrp_len, 0, buf, sizeof(buf), &output_len);
                                  ...
                                  ms_node *node, ms_node *parent)
{
    ...
    /* check bip32 key */
    if ((path = memchr(node->data, '/', node->data_len))) {
        path_len = node->data_len - (path - node->data) - 1;
        node->data_len = path - node->data; /* Trim node data to just the bip32 key */
        ++path; /* Skip leading '/' */
        if (path == node->data + node->data_len)
            path = NULL; /* Empty derivation path */
        else if (*path == '/')
    ...
    if (path) {
        ...
        ret = bip32_path_from_string_n(path, path_len,
                                       node->kind == KIND_BIP32_PRIVATE_KEY,
                                       child_path, NUM_ELEMS(child_path),
                                ...
                                unsigned char *script, size_t script_len, size_t *written)
{
    ...
    *written = 0;
    ...
    parent = p->parent;
    p->parent = NULL;
    ret = generate_script(p, child_num, buf, DESCRIPTOR_LIMIT_LENGTH, &output_len);
    p->parent = parent;
                                  ...
                                  char **addresses, size_t num_addresses)
{
    char *str;
    const struct addr_ver_t *addr_ver = addr_ver_from_network(network);
    struct ms_context ctx = { NULL, DESCRIPTOR_LIMIT_LENGTH, child_num, num_addresses };
    int ret;
    ...
    if ((ret = wally_descriptor_canonicalize(descriptor, vars_in, 0, &str)) == WALLY_OK) {
        if (!(ctx.script = wally_malloc(DESCRIPTOR_LIMIT_LENGTH))) {
            wally_free_string(str);
            return WALLY_ENOMEM;
        }
        ret = parse_miniscript(str, strlen(str), flags, KIND_MINISCRIPT | KIND_DESCRIPTOR,
    ...
    clear_and_free(ctx.script, DESCRIPTOR_LIMIT_LENGTH);
    wally_free_string(str);
    return ret;
}
                                  ...
                                  char **output)
{
    ...
    while (*p && *p != '#') {
        ...
        if (p != start) {
            ...
            if (!vars_in || lookup_len > VAR_MAX_NAME_LEN || is_number) {
                required_len += lookup_len; /* Too long to be an identifier */
            } else {
    ...
    *out++ = '#';
    ...
    if (generate_checksum(*output, required_len, out) != WALLY_OK ||
        (*p == '#' && strcmp(p + 1, out))) {
        /* Invalid character in input or failed to match descriptor */
